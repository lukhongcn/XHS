...
namespace XHS.BLL
{
    ...
    public class PackingOperationService
    {
        ...
        /// <summary>完成装箱（事务内完成）。</summary>
        public PackingOperationResult CompletePacking(
            long packingId,
            string pageToken,
            string userName)
        {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlTransaction transaction = connection.BeginTransaction())
                    {
                        try
                        {
                            ...
                            {
                                transaction.Rollback();
                                return PackingOperationResult.Lock("装箱已异常锁定，不能完成装箱。", record.LockToken, record);
                            }
                            ...
                            if (record.Status == 1)
                            {
                                transaction.Rollback();
                                record = LoadPackingRecord(packingId);
                                return PackingOperationResult.Error("装箱已完成，不能重复完成。");
                            }

                            // 5. 从扫描明细汇总数量
                            int totalScanned = packingScanRecordDal.GetScannedMaterialQty(packingId, connection, transaction);

                            // 6. 确认数量等于计划数量
                            if (totalScanned != (record.PlanQty ?? 0))
                            {
                                transaction.Rollback();
                                return PackingOperationResult.Error(string.Format("已扫描数量({0})与计划数量({1})不一致，不能完成装箱。", totalScanned, record.PlanQty ?? 0));
                            }
                            ...
                            record = LoadPackingRecord(packingId);
                            return PackingOperationResult.Completed("装箱已完成。", newToken, record);
                        }
                        catch
                        {
                            transaction.Rollback();
