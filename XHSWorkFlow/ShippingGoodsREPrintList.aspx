<%@ Page Language="c#" CodeBehind="ShippingGoodsREPrintList.aspx.cs" AutoEventWireup="false" Inherits="ModuleWorkFlow.ShippingGoodsREPrintList" MasterPageFile="~/DefaultSub.Master" %>
                    ...
                    <div class="menubox">
                        <div class="mod2">
                            <ul>
                                ...
                                <li class="btn9"><asp:LinkButton ID="lnkbutton_print" runat="server" ToolTip="补打/reprint" OnClick="lnkbutton_print_Click" OnClientClick="return preparePrintClientId();">补打/reprint</asp:LinkButton></li>
                            </ul>
                        </div>
                        <div class="clearbox"></div>
                    </div>
                            ...
                            <Columns>
                                <asp:TemplateColumn HeaderText="选择">
                                    ...
                                    <ItemTemplate>
                                        ...
                                        <asp:HiddenField ID="hid_row_id" runat="server" Value='<%# Eval("Id") %>' />
                                    </ItemTemplate>
                                </asp:TemplateColumn>
                                <asp:TemplateColumn HeaderText="原因">
                                    <HeaderTemplate>
                                        <asp:DropDownList ID="DropDownList_HeaderReprintReason" runat="server" CssClass="form-select text-start border-primary"></asp:DropDownList>
                                    </HeaderTemplate>
                                    <ItemTemplate>
                                        <asp:DropDownList ID="DropDownList_ReprintReason" runat="server" CssClass="form-select text-start border-primary"></asp:DropDownList>
                                    </ItemTemplate>
                                </asp:TemplateColumn>
...
<asp:Content ID="Content2" ContentPlaceHolderID="JSHolder" runat="server">
    ...
    <script type="text/javascript">
        ...
        function applyHeaderReprintReason(control) {
            if (!control) {
                return;
            }
            ...
            for (var i = 0; i < form.elements.length; i++) {
                ...
                if (!element || element.type !== 'checkbox' || !element.checked || !element.id || element.id.indexOf('chk_datagrid') < 0) {
                    continue;
                }
                ...
                for (var j = 0; j < selects.length; j++) {
                    if (selects[j].id && selects[j].id.indexOf('DropDownList_ReprintReason') >= 0) {
                        selects[j].value = control.value;
                    }
                }
            }
        }
